...
class Transcript(object):
    ...
    def output(self, outfile, outfile_list):
        out = self.ENST + '\t' + self.HGNC + '\t' + self.GENE + '\t' + self.ENSG + '\t' + self.getInfoString() + '\t' + self.CHROM + '\t' + self.STRAND + '\t' + str(self.POS)
        ...
        outfile.write(out + '\n')
        outfile_list.write(self.HGNC+'\t'+self.ENSG+'\t'+self.GENE+'\t'+self.ENST+'\n')
...
def readGeneNames():
    ret = dict()
    for line in gzip.open('genes.txt.gz'):
        line = line.strip()
        if line == '' or line.startswith('#'): continue
        cols = line.split('\t')
        ensg = cols[13]
        info = cols[10]
        if 'HGNC:' not in info: continue
        hgnc = info[info.find('HGNC:'):-1]
        ret[ensg] = hgnc
    return ret

# Process Ensembl data
def run(options, genome_build):
    ...
    url2 = 'ftp://ftp.ensembl.org/pub/release-' + options.ensembl + '/mysql/homo_sapiens_core_' + options.ensembl + '_' + genome_build[-2:] + '/gene.txt.gz'
    try:
        urllib.urlretrieve(url, 'ensembl_data.gz')
        urllib.urlretrieve(url2, 'genes.txt.gz')
    except:
    ...
    for line in gzip.open('ensembl_data.gz', 'r'):
        ...
        if not enst == prevenst:
            ...
            if transcript.ENSG in geneNames: transcript.HGNC = geneNames[transcript.ENSG]
            else: transcript.HGNC = '.'
    ...
    outfile_list.write('# Created by ensembl_prep based on Ensembl release '+options.ensembl+' (genome build '+genome_build+')\n')
    outfile_list.write('HGNCID\tENSG\tSYMBOL\tENST\n')
    ...
    for line in open('temp.txt'):
        ...
        if record[5] in data.keys():
            data[record[5]].append(record)
        else:
            data[record[5]] = []
            data[record[5]].append(record)
    ...
    sys.stdout.flush()
    sortedRecords = sortRecords(data, 7, 8)
...
if __name__ == '__main__':
    ...
    descr = 'ensembl_prep is a simple tool for generating Ensembl transcript database for transcript_map.'
    epilog = '\nExample usage: ./ensembl_prep.py -i input.txt -e 70 -o out -s\n\n'
    OptionParser.format_epilog = lambda self, formatter: self.epilog
    parser = OptionParser(usage='python path/to/cava/ensembl_prep.py <options>', description=descr,epilog=epilog)
    ...
    print "\n---------------------------------------------------------------------------------------"
    print 'ensembl_prep for transcript_map is now running'
    ...
    Nretrieved = run(options, genome_build)
    print '\nA total of ' + str(Nretrieved) + ' transcripts have been retrieved\n'

    # Compressing output file with Tabix
